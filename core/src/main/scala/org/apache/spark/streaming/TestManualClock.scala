...
class TestManualClock(var time: Long) extends Clock {
  ...
  /**
   * @return
   *   `ManualClock` with initial time 0
   */
  def this() = this(0L)
  ...
  /**
   * @param timeToAdd
   *   time (in milliseconds) to add to the clock's time
   */
  ...
  /**
   * @param targetTime
   *   block until the clock time is set or advanced to at least this time
   * @return
   *   current time reported by the clock when waiting finishes
   */
