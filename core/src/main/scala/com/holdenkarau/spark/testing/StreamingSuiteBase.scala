...
/**
 * This is the base trait for Spark Streaming testsuites. This provides basic functionality to run user-defined set of input on user-defined stream
 * operations, and verify the output.
 */
trait StreamingSuiteBase extends BeforeAndAfterAll with Logging with StreamingSuiteCommon with SharedSparkContext {
  ...
  /**
   * Test binary DStream and RDD operation with two lists of inputs, with number of batches to run same as the number of input values corresponding to
   * the DStream.
   *
   * Each input micro-batch is a list of values or as null to simulate empty batch.
   *
  ...
  def testOperationWithRDD[U: ClassTag, V: ClassTag, W: ClassTag](
    ...
    withOutputAndStreamingContext(setupStreamAndRDD[U, V, W](input1, input2, operation)) { (outputStream, ssc) =>
      val output = runStreams[W](outputStream, ssc, numBatches, expectedOutput.size)
      verifyOutput[W](output, expectedOutput, ordered)
    }
  }
