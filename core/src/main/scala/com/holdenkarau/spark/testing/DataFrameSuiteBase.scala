...
trait DataFrameSuiteBaseLike extends SparkContextProvider with TestSuiteLike with Serializable {
  ...
  def sqlBeforeAllTestCases(): Unit = {
    if (
      (SparkSessionProvider._sparkSession ne null) &&
      ...
      def newBuilder()       = {
        ...
        builder.config("spark.sql.streaming.checkpointLocation", Utils.createTempDir().toPath().toString)
  ...
  /**
   * Compares if two [[DataFrame]]s are equal, checks the schema and then if that matches checks if the rows are equal.
   */
  ...
  /**
   * Compares if two [[DataFrame]]s are equal without caring about order of rows, by finding elements in one DataFrame that is not in the other. The
   * resulting DataFrame should be empty inferring the two DataFrames have the same elements. Also verifies that the schema is identical.
   */
  def assertDataFrameNoOrderEquals(expected: DataFrame, result: DataFrame): Unit = {
    assert(expected.schema, result.schema)
  ...
  /**
   * Compares if two [[DataFrame]]s are equal without caring about order of rows, by finding elements in one DataFrame that is not in the other. The
   * resulting DataFrame should be empty inferring the two DataFrames have the same elements. Does not compare the schema.
   */
  def assertDataFrameDataEquals(expected: DataFrame, result: DataFrame): Unit = {
    ...
    try {
      expected.rdd.cache()
      result.rdd.cache()
      assert("Column size not Equal", expected.columns.length, result.columns.length)
      assert("Length not Equal", expected.rdd.count(), result.rdd.count())

      val columns               = expected.columns.map(s => col(s))
      val expectedElementsCount = expected
        .groupBy(columns.toIndexedSeq: _*)
