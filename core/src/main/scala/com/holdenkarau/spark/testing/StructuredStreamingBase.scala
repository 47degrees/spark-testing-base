 ...
 */
trait StructuredStreamingBase extends DataFrameSuiteBase with StructuredStreamingBaseLike { self: Suite =>
...
trait StructuredStreamingBaseLike extends SparkContextProvider with TestSuiteLike with Serializable {
  ...
  private[holdenkarau] def runSimpleStreamEndState[T: Encoder, R: Encoder](
    ...
    count = count + 1
    val query               = transformed.writeStream.format("memory").outputMode(mode).queryName(queryName).start()
