 ...
 */
// tag::collectResults[]
class TestOutputStream[T: ClassTag](parent: DStream[T], val output: ArrayBuffer[Seq[T]] = ArrayBuffer[Seq[T]]()) extends Serializable {

  parent.foreachRDD { (rdd: RDD[T], time) =>
    val collected = rdd.collect()
    output += collected.toIndexedSeq
  }
...
private[holdenkarau] trait StreamingSuiteCommon extends Logging with SparkContextProvider {
   ...
   */
  private[holdenkarau] def createTestInputStream[T: ClassTag](sc: SparkContext, ssc_ : TestStreamingContext, input: Seq[Seq[T]]): TestInputStream[T] =
  ...
  private[holdenkarau] def setupStreams[U: ClassTag, V: ClassTag](
    ...
    val operatedStream = operation(inputStream) // TODO - Check alternative
    val outputStream   = new TestOutputStream[V](operatedStream, new ArrayBuffer[Seq[V]] /* with SynchronizedBuffer[Seq[V]]*/ )
    (outputStream, ssc)
  }
  ...
  private[holdenkarau] def setupStreams[U: ClassTag, V: ClassTag, W: ClassTag](
    ...
    val operatedStream = operation(inputStream1, inputStream2) // TODO - Check alternative
    val outputStream   = new TestOutputStream[W](operatedStream, new ArrayBuffer[Seq[W]] /* with SynchronizedBuffer[Seq[V]]*/ )
    (outputStream, ssc)
  }

  /**
   * Set up required DStream and RDD to test the binary operation using the sequence of input collections and values.
   */
  private[holdenkarau] def setupStreamAndRDD[U: ClassTag, V: ClassTag, W: ClassTag](
    ...
    val operatedStream = operation(inputStream1, inputRDD2) // TODO - Check alternative
    val outputStream   = new TestOutputStream[W](operatedStream, new ArrayBuffer[Seq[W]] /* with SynchronizedBuffer[Seq[V]]*/ )
    (outputStream, ssc)
  }

  /**
   * Runs the streams set up in `ssc` on manual clock for `numBatches` batches and returns the collected output. It will wait until
   * `numExpectedOutput` number of output data has been collected or timeout (set by `maxWaitTimeMillis`) is reached.
   *
  ...
  private[holdenkarau] def runStreams[V: ClassTag](
    ...
    assert(numExpectedOutput > 0, s"Number of expected outputs after ${numBatches} is zero")
    ...
    assert(timeTaken < maxWaitTimeMillis, s"Operation timed out after ${timeTaken} ms")
