...
trait StreamingActionBase extends StreamingSuiteBase {
  ...
  private def withStreamingContext(outputStreamSSC: TestStreamingContext)(block: TestStreamingContext => Unit): Unit = {
    try block(outputStreamSSC)
    finally {
      try outputStreamSSC.stop(stopSparkContext = false)
      catch {
        case e: Throwable =>
  ...
  private def setupStream[U: ClassTag](input: Seq[Seq[U]], operation: DStream[U] => Any): TestStreamingContext = {
  ...
  private def runActionStream(ssc: TestStreamingContext, numBatches: Int): Unit = {
    ...
    while (
      batchCountListener.batchCount < numBatches &&
      System.currentTimeMillis() - startTime < maxWaitTimeMillis
    ) {
      logInfo(
        s"batches: run = ${batchCountListener.batchCount} " +
          s"target = ${numBatches}"
      )
