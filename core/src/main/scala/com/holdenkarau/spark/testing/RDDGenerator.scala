...
object RDDGenerator {
  ...
  /**
   * Generate an RDD of the desired type. Attempt to try different number of partitions so as to catch problems with empty partitions, etc.
   *
   * minPartitions defaults to 1, but when generating data too large for a single machine you must choose a larger value.
   *
   * @param sc
   *   Spark Context
   * @param minPartitions
  ...
  /**
   * Generate an RDD of the desired type with its size accessible in the lambda. Attempt to try different number of partitions so as to catch problems
   * with empty partitions, etc.
   *
   * minPartitions defaults to 1, but when generating data too large for a single machine you must choose a larger value.
   *
   * @param sc
   ...
   * @return
   */
  def genSizedRDD[T: ClassTag](sc: SparkContext, minPartitions: Int = 1)(generator: Int => Gen[T]): Gen[RDD[T]] =
  ...
  def arbitraryRDD[T: ClassTag](sc: SparkContext, minPartitions: Int = 1)(generator: => Gen[T]): Arbitrary[RDD[T]] = {
    Arbitrary {
      Gen.sized(sz => arbitrary[T](sc, minPartitions, sz)(new WrappedGenerator[T](generator)))
    }
  }
   ...
   * @return
   */
  def arbitrarySizedRDD[T: ClassTag](sc: SparkContext, minPartitions: Int = 1)(generator: Int => Gen[T]): Arbitrary[RDD[T]] = {
    Arbitrary {
      Gen.sized { sz =>
        arbitrary[T](sc, minPartitions, sz)(new WrappedSizedGenerator[T](sz, generator))
      }
    }
  }

  private def arbitrary[T: ClassTag](sc: SparkContext, minPartitions: Int, sz: Int)(randomDataGenerator: => RandomDataGenerator[T]): Gen[RDD[T]] = {
    sz match {
 ...
 */
private[testing] class WrappedGenerator[T](getGenerator: => Gen[T]) extends WrappedGen[T] {
...
private[testing] class WrappedSizedGenerator[T](size: Int, getGenerator: Int => Gen[T]) extends WrappedGen[T] {
