...
/**
 * This is the base trait for Spark Streaming testsuite. This provides basic functionality to run user-defined set of input on user-defined stream
 * operations, and verify the output matches as expected.
 *
...
class JavaStreamingSuiteBase extends JavaSuiteBase with StreamingSuiteCommon {
  ...
  /**
   * Verify whether the output values after running a DStream operation is same as the expected output values, by comparing the output collections
   * either as lists (order matters) or sets (order does not matter)
   */
  def verifyOutput[V: ClassTag](output: Seq[Seq[V]], expectedOutput: Seq[Seq[V]], ordered: Boolean): Unit = {
    ...
    for (i <- output.indices) {
      if (ordered) {
        // TODO - Review
        expectedOutput(i).toList == output(i).toList
      } else {
        // TODO - Review
        expectedOutput(i).groupBy(identity).view.mapValues(_.size).toSet ==
          output(i).groupBy(identity).view.mapValues(_.size).toSet
      }
    }
   ...
   */
  def testOperation[U, V](input: JList[JList[U]], operation: JFunction[JavaDStream[U], JavaDStream[V]], expectedOutput: JList[JList[V]]): Unit =
   ...
   *   Sequence of expected output collections
   * @param ordered
   *   Compare output values with expected output values within the same output batch ordered or unordered. Comparing doubles may not work well in
   *   case of unordered.
   */
  def testOperation[U, V](
    ...
    withOutputAndStreamingContext(setupStreams[U, V](sInput, wrappedOperation)) { (outputStream, ssc) =>
  ...
  /**
   * Test binary DStream operation with two lists of inputs, with number of batches to run same as the number of input values. The size of the two
   * input lists should be equal.
   *
   * Each input micro-batch is a list of values or as null to simulate empty batch.
   *
   * @param input1
  ...
  def testOperation[U, V, W](
    ...
    assertEquals("Length of the input lists are not equal", input1.asScala.length, input2.asScala.length)
    ...
    withOutputAndStreamingContext(setupStreams[U, V, W](sInput1, sInput2, wrappedOperation)) { (outputStream, ssc) =>
