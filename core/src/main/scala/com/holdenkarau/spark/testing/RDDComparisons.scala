...
trait RDDComparisonsLike extends TestSuiteLike {
  ...
  /**
   * Compare two RDDs with order (e.g. [1,2,3] != [3,2,1]) If the partitioners are not the same this requires multiple passes on the input. If they
   * are equal returns None, otherwise returns Some with the first mismatch. If the lengths are not equal, one of the two components may be None.
   */
  def compareRDDWithOrder[T: ClassTag](expected: RDD[T], result: RDD[T]): Option[(Option[T], Option[T])] = {
    ...
    if (result.partitioner.map(_ == expected.partitioner.get).getOrElse(false)) {
      ...
      indexedExpected
        .cogroup(indexedResult).filter { case (_, (i1, i2)) =>
          i1.isEmpty || i2.isEmpty || i1.head != i2.head
        }.take(1).headOption.map { case (_, (i1, i2)) =>
   ...
   *
   * @return
   *   None if the two RDDs are equal, or Some containing the first mismatch information. The mismatch information will be Tuple3 of: (key, number of
   *   times this key occur in expected RDD, number of times this key occur in result RDD)
   */
  def compareRDD[T: ClassTag](expected: RDD[T], result: RDD[T]): Option[(T, Int, Int)] = {
    ...
    expectedKeyed
      .cogroup(resultKeyed).filter { case (_, (i1, i2)) =>
        i1.isEmpty || i2.isEmpty || i1.head != i2.head
      }
      .take(1).headOption.map { case (v, (i1, i2)) =>
