...
object Utils extends Logging {
  ...
  private def cleanupPath(dirPath: String) = {
    try Utils.deleteRecursively(new File(dirPath))
    catch {
  ...
  /**
   * Delete a file or directory and its contents recursively. Don't follow directories if they are symlinks. Throws an exception if deletion is
   * unsuccessful.
   */
  ...
  def createDirectory(root: String): File = {
    ...
    while (dir == null) {
      ...
      if (attempts > maxAttempts) {
        throw new IOException(s"Failed to create a temp directory (under ${root}) after ${maxAttempts}")
      }
      try {
  ...
  /**
   * Attempt to start a service on the given port, or fail after a number of attempts. Each subsequent attempt uses 1 + the port used in the previous
   * attempt (unless the port is 0).
   *
   * @param startPort
   *   The initial port to start the service on.
   * @param startService
   *   Function to start service on a given port. This is expected to throw java.net.BindException on port collision.
   * @param conf
   *   A SparkConf used to get the maximum number of retries when binding to a port.
   * @param serviceName
   *   Name of the service.
   */
  def startServiceOnPort[T](startPort: Int, startService: Int => (T, Int), conf: SparkConf, serviceName: String = ""): (T, Int) = {
