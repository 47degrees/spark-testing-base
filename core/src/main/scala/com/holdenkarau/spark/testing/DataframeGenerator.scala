...
object DataframeGenerator {
  ...
  /**
   * Creates a DataFrame Generator for the given Schema, and the given custom generators. Custom generators should be specified as a list of: (column
   * index, generator function) tuples.
   *
   * Note: The given custom generators should match the required schema, for ex. you can't use Int generator for StringType.
   *
   * Note 2: The ColumnGenerator* accepted as userGenerators has changed. ColumnGenerator is now the base class of the accepted generators, users
   * upgrading to 0.6 need to change their calls to use Column. Further explanation can be found in the release notes, and in the class descriptions
  ...
  def arbitraryDataFrameWithCustomFields(sqlContext: SQLContext, schema: StructType, minPartitions: Int = 1)(
      userGenerators: ColumnGenerator*
  ): Arbitrary[DataFrame] = {
    ...
    fields.toList.map { field =>
      if (generatorMap.contains(field.name)) {
        generatorMap.get(field.name) match {
          ...
          case _ => getGenerator(field.dataType, nullable = field.nullable)
        }
      } else getGenerator(field.dataType, nullable = field.nullable)
    }
  }

  private def getGenerator(dataType: DataType, generators: Seq[ColumnGenerator] = Seq(), nullable: Boolean = false): Gen[Any] = {
    val nonNullGen = dataType match {
      ...
      case TimestampType                =>
        Arbitrary.arbitrary[Long].map { l =>
          new Timestamp(l / 10000)
        }
      case DateType                     =>
        Arbitrary.arbLong.arbitrary.map { l =>
          new Date(l / 10000)
        }
