...
object JavaRDDComparisons extends RDDComparisonsLike with JavaTestSuite {
   ...
   *
   * @return
   *   None if the two RDDs are equal, or Some that contains the first mismatch information. Mismatch information will be Tuple3 of: (key, number of
  ...
  def compareRDD[T](expected: JavaRDD[T], result: JavaRDD[T]): Option[(T, Integer, Integer)] = {
    implicit val ctag = Utils.fakeClassTag[T]
    compareRDD(expected.rdd, result.rdd).map { case (value, expectedCount, resultCount) =>
      (value, Integer.valueOf(expectedCount), Integer.valueOf(resultCount))
    }
  }
