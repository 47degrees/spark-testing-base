...
class JavaDatasetSuiteBase extends JavaDataFrameSuiteBase with DatasetSuiteBaseLike with Serializable {
  ...
  /**
   * Check if two Datasets are equals, Datasets should have the same type. This method could be customized by overriding equals method for the given
   * class type.
   */
   ...
   */
  def assertDatasetApproximateEquals[U](expected: Dataset[U], result: Dataset[U], tol: Double): Unit =
...
trait DatasetSuiteBaseLike extends DataFrameSuiteBaseLike {
  ...
  def assertDatasetEquals[U](expected: Dataset[U], result: Dataset[U])(implicit UCT: ClassTag[U]): Unit = {
    try {
      ...
      val unequalRDD                         = expectedIndexValue.join(resultIndexValue).filter { case (idx, (o1, o2)) => o1 != o2 }
