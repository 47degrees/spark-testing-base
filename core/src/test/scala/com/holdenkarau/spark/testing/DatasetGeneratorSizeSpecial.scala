...
class DatasetGeneratorSizeSpecial extends FunSuite with SharedSparkContext with Checkers {
  ...
  test("test generating sized Datasets[Custom Class]") {
    ...
    val carGen: Gen[Dataset[Seq[Car]]] =
      DatasetGenerator.genSizedDataset[Seq[Car]](sqlContext) { size =>
        ...
        } yield slowCars ++ normalSpeedCars
        cars
      }

    val property =
      forAll(carGen.map(_.flatMap(identity))) { dataset =>
