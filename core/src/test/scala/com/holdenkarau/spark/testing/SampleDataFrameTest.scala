...
class SampleDataFrameTest extends FunSuite with DataFrameSuiteBase {
  ...
  test("unequal dataframes should not be equal when byte array differs") {
    import sqlContext.implicits._
    val input         = sc.parallelize(inputList).toDF
    val diffInputList = List(Magic("panda", 9001.0, byteArray), Magic("coffee", 9002.0, diffByteArray))
  ...
  test("equal dataframes with different field order should be equal") {
    val df1 = rowDf("a" -> (IntegerType, 1), "b" -> (StringType, "a string"))
    val df2 = rowDf("b" -> (StringType, "a string"), "a" -> (IntegerType, 1))
    assertDataFrameDataEquals(df1, df2)
  }

  test("assertDataFrameDataEquals fails dataframes with different columns size") {
    val df1    = rowDf("a" -> (IntegerType, 1), "b" -> (StringType, "a string"))
    val df2    = rowDf("b" -> (StringType, "a string"), "a" -> (IntegerType, 1), "c" -> (IntegerType, 1))
  ...
  test("equal DF of rows of bytes should be equal (see GH issue #247)") {
    val df = rowDf("a" -> (BinaryType, "bytes".getBytes()), "b" -> (StringType, "good"))
