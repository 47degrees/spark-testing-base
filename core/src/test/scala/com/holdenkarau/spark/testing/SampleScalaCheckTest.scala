...
class SampleScalaCheckTest extends FunSuite with SharedSparkContext with RDDComparisons with Checkers {
  ...
  test("map should not change number of elements") {
    val property =
      forAll(RDDGenerator.genRDD[String](sc)(Arbitrary.arbitrary[String])) { rdd =>
  ...
  test("assert that two methods on the RDD have the same results") {
    val property =
      forAll(RDDGenerator.genRDD[String](sc)(Arbitrary.arbitrary[String])) { rdd =>
        compareRDD(filterOne(rdd), filterOther(rdd)).isEmpty
      }
  ...
  test("test RDD sized generator (nodes)") {
    ...
    val nodeGenerator = RDDGenerator.genSizedRDD[(Int, MyNode)](sc) { size: Int =>
      ...
      } yield (size, MyNode(id, nodes.map { case (_, node) => node }))
      genGraph(size)
    }

    val property =
      forAll(nodeGenerator) { rdd =>
  ...
  test("test custom RDD sized generator") {
    ...
    val property =
      forAll(humanGen.map(_.flatMap(identity))) { rdd =>
  ...
  test("assert rows' types like schema type") {
    val schema           = StructType(List(StructField("name", StringType, nullable = false), StructField("age", IntegerType, nullable = false)))
    val rowGen: Gen[Row] = DataframeGenerator.getRowGenerator(schema)
    val property         =
      forAll(rowGen) { row =>
        row.get(0).isInstanceOf[String] && row.get(1).isInstanceOf[Int]
      }
  ...
  test("test custom columns generators") {
    ...
    val dataframeGen =
      DataframeGenerator.arbitraryDataFrameWithCustomFields(sqlContext, schema)(ageGenerator)
  ...
  test("test multiple columns generators") {
    ...
    val ageGenerator  = new Column("age", Gen.choose(10, 100))
    val dataframeGen  =
      DataframeGenerator.arbitraryDataFrameWithCustomFields(sqlContext, schema)(nameGenerator, ageGenerator)
  ...
  test("test multi-level column generators") {
    ...
    val sqlContext    = SparkSession.builder.getOrCreate().sqlContext
    val userGenerator = new ColumnList(
      "user",
      Seq(
        // name should be on of Holden or Hanafy
    ...
    val dataframeGen  =
      DataframeGenerator.arbitraryDataFrameWithCustomFields(sqlContext, schema)(userGenerator)

    val sqlExpr  = """
                 |(user.name != 'Holden' AND user.name != 'Hanafy') OR
                 |(user.age > 100 OR user.age < 10) OR
                 |(user.address.zip_code > 200 OR user.address.zip_code < 100)""".stripMargin
    val property =
      forAll(dataframeGen.arbitrary) { dataframe =>
        dataframe.schema === schema &&
        dataframe.filter(sqlExpr).count() == 0
  ...
  test("assert invalid ColumnList does not compile") {
    ...
    assertTypeError("""
                    |val userGenerator =
                    |  new ColumnList("user",
                    |    Seq("list", "of", "an", "unsupported", "type"))""".stripMargin)
  }

  test("generate rdd of specific size") {
    implicit val generatorDrivenConfig =
      PropertyCheckConfig(minSize = 10, maxSize = 20)
    val prop                           = forAll(RDDGenerator.genRDD[String](sc)(Arbitrary.arbitrary[String])) { rdd =>
      rdd.count() <= 20
    }
    check(prop)
  }
  ...
  test("test simple Array Type generator") {
    val schema = StructType(List(StructField("name", StringType, true), StructField("pandas", ArrayType(FloatType, false))))
  ...
  test("decimal generation") {
    val schema =
      StructType(List(StructField("small", DecimalType(3, 1)), StructField("medium", DecimalType(15, 8)), StructField("large", DecimalType(38, 38))))
